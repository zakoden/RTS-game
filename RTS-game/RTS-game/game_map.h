...
private:
	...
	const uint32_t BLOCKS_IN_LINE = 4; // количество блоков в файле с текстурой

    uint32_t width_, height_; // ширина, высота карты
	std::vector<uint8_t> blocks_; // блоки карты
	std::vector<std::unordered_set<AbstractUnit*>> units_in_block_; // юниты на карте
	SDL_Texture* tiles_ = nullptr; // текстура со всеми блоками
