...
void BehaviorScout::DoAction() {
	MapLayer* cur_layer = unit_->GetCurrentLayer();
	if (target_.x == -1 ||
		cur_layer->IsCellUncovered(adapter::UnitToMapCoordinates(target_), unit_->GetPlayer())) {  // No target
		...
		for (uint32_t i = 0; i < cur_layer->GetHeight(); ++i) {
			for (uint32_t j = 0; j < cur_layer->GetWidth(); ++j) {
				if (!cur_layer->IsCellUncovered({j, i}, unit_->GetPlayer())) {
					float score =
						COEF_DISTANCE_FROM_BASE * cur_layer->GetDistanceToBase(unit_->GetPlayer())[i][j]
