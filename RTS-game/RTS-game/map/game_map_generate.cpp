...
void GameMap::Generate() {
	...
	uint32_t height = GetHeight(), width = GetWidth();
	const size_t TOTAL_AREA = static_cast<size_t>(height) * width;
	...
	*/
	...
	// Generate how many cells will be water and mountain
	// Where percentile will be generated, e.g. WATER_PERCENTILE will be between [W_P_L; W_P_R]
	// Percentile means that there will be WATER_DEEP_PERCENTILE% of deep water, WATER_PERCENTILE% of water and deep water, etc.

	
	/* {
		const size_t WATER_DEEP_AREA = GetRandomArea(5, 7, TOTAL_AREA);
		const size_t WATER_AREA = GetRandomArea(11, 13, TOTAL_AREA);
		const size_t WATER_SHALLOW_AREA = GetRandomArea(17, 20, TOTAL_AREA);
		const size_t MOUNTAIN_HIGH_AREA = GetRandomArea(5, 8, TOTAL_AREA);
		const size_t MOUNTAIN_LOW_AREA = GetRandomArea(15, 17, TOTAL_AREA);

		vector<Triple> points_by_height(TOTAL_AREA);
		for (uint32_t i = 0; i < height; ++i)
			for (uint32_t j = 0; j < width; ++j)
				points_by_height[GetInd(j, i)] = { heights[i][j], {j, i} };
		sort(points_by_height.begin(), points_by_height.end());

		for (int i = 0; i < WATER_SHALLOW_AREA; ++i) {
			if (i < WATER_DEEP_AREA)
				blocks[points_by_height[i].point] = WATER_DEEP;
			else if (i < WATER_AREA)
				blocks[points_by_height[i].point] = WATER;
			else
				blocks[points_by_height[i].point] = WATER_SHALLOW;
		}

		for (size_t i = TOTAL_AREA - 1; i >= TOTAL_AREA - MOUNTAIN_LOW_AREA; --i) {
			if (i >= TOTAL_AREA - MOUNTAIN_HIGH_AREA)
				blocks[points_by_height[i].point] = MOUNTAIN_HIGH;
			else
				blocks[points_by_height[i].point] = MOUNTAIN_LOW;
		}
	} */
	{
		const float MOUNTAIN_LOW_HEIGHT = 0.2f, MOUNTAIN_HIGH_HEIGHT = 0.25f;
		...
		for (uint32_t i = 0; i < height; ++i)
			for (uint32_t j = 0; j < width; ++j)
				average_height += heights[i][j];
		average_height /= height * width;

		for (uint32_t i = 0; i < height; ++i) {
			for (uint32_t j = 0; j < width; ++j) {
				heights[i][j] -= average_height;
	...
	*/
	...
	for (uint32_t i = 0; i < height; ++i) {
		for (uint32_t j = 0; j < width; ++j) {
			...
			mountain_area += (blocks[i][j] == MOUNTAIN_HIGH || blocks[i][j] == MOUNTAIN_LOW);
		}
	}
