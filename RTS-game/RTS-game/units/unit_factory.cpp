...
void UnitFactory::SetCurrentLayerIndex(uint8_t cur_layer_ind) {
	cur_layer_ind_ = cur_layer_ind;
}

MovableUnit* UnitFactory::CreateTest(size_t player, int x, int y) {
	Unit* unit = new Unit(10, 1, 35, 0.5, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateTest1(size_t player, int x, int y) {
	Unit* unit = new Unit(10, 1, 35, 1.0, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateTestHunter(size_t player, int x, int y) {
	Unit* unit = new Unit(10 + rand() % 5, 1, 40, 1.0, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateTestHunter2(size_t player, int x, int y) {
	Unit* unit = new Unit(10, 1, 50 + rand() % 20, 0.5, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateFireSmallPoleax(size_t player, int x, int y) {
	Unit* unit = new Unit(15, 1, 50 + rand() % 20, 0.7, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateFireSmallSpear(size_t player, int x, int y) {
	Unit* unit = new Unit(17, 1, 50 + rand() % 20, 0.7, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateFireSmallLance(size_t player, int x, int y) {
	Unit* unit = new Unit(14, 5, 70, 0.7, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit* UnitFactory::CreateFireMediumPoleax(size_t player, int x, int y) {
	Unit* unit = new Unit(30, 5, 150, 0.85, texture_manager_, game_map_, cur_layer_ind_);
...
MovableUnit * UnitFactory::CreateScout(size_t player, int x, int y, MapPoint center) {
	Unit* unit = new Unit(4, 1, 40 + rand() % 20, 2, texture_manager_, game_map_, cur_layer_ind_);
	unit->SetTexture(2, 2, 2, 10, 10, 3, 2, 10, 14, 16, 16);
...
MovableUnit* UnitFactory::CreateBulletFire1(size_t player, int x, int y, int x_to, int y_to) {
	Unit* unit = new Unit(10, 1, 1, 5.0, texture_manager_, game_map_, cur_layer_ind_);
...
Building* UnitFactory::CreateBase(uint8_t player, int x, int y) {
	...
	game_map_->GetLayer(cur_layer_ind_)->AddBase({ x, y }, player);
	return building;
}
